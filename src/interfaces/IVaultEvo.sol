...
import {ICanal, Id, MarketParams} from "@canal/interfaces/ICanal.sol";
...
interface IVaultEvoBase {
    /// @notice The address of the Canal contract.
    function CANAL() external view returns (ICanal);
...
/// @dev This interface is inherited by VaultEvo so that function signatures are checked by the compiler.
/// @dev Consider using the IVaultEvo interface instead of this one.
interface IVaultEvoStaticTyping is IVaultEvoBase {
...
interface IVaultEvo is IVaultEvoBase, IERC4626, IERC20Permit, IOwnable, IMulticall {
    /// @notice Returns the current configuration of each market.

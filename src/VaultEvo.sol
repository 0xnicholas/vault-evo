...
Sections
    ...
    3. CONSTRUCTOR
    4. MODIFIERS
    5. ONLY OWNER FUNCTIONS
    6. ONLY CURATOR FUNCTIONS
    7. ONLY ALLOCATOR FUNCTIONS
    8. REVOKE FUNCTIONS
    9. EXTERNAL
    10. ERC4626 (PUBLIC)
    11. ERC4626 (INTERNAL)
    12. INTERNAL
    13. LIQUIDITY ALLOCATION
    14. FEE MANAGEMENT
 */


contract VaultEvo is ERC4626, ERC20Permit, Ownable2Step, Multicall, IVaultEvoStaticTyping {
    ...
    constructor(
        ...
        IERC20(_asset).forceApprove(canal, type(uint256).max);
    }
    ...
    function submitCap(MarketParams memory marketParams, uint256 newSupplyCap) external onlyCuratorRole {
        ...
        if (CANAL.lastUpdate(id) == 0) revert ErrorsLib.MarketNotCreated();
    ...
    function updateWithdrawQueue(uint256[] calldata indexes) external onlyAllocatorRole {
        ...
        for (uint256 i; i < currLength; ++i) {
            if (!seen[i]) {
                ...
                if (CANAL.supplyShares(id, address(this)) != 0) {
        ...
        for (uint256 i; i < allocations.length; ++i) {
            ...
            if (withdrawn > 0) {
                ...
                (uint256 withdrawnAssets, uint256 withdrawnShares) =
                    CANAL.withdraw(allocation.marketParams, withdrawn, shares, address(this), address(this));
                ...
                (, uint256 suppliedShares) =
                    CANAL.supply(allocation.marketParams, suppliedAssets, 0, address(this), hex"");
    ...
    function totalAssets() public view override returns (uint256 assets) {
        for (uint256 i; i < withdrawQueue.length; ++i) {
            assets += CANAL.expectedSupplyAssets(_marketParams(withdrawQueue[i]), address(this));
        }
    }
    ...
    function _maxDeposit() internal view returns (uint256 totalSuppliable) {
        for (uint256 i; i < supplyQueue.length; ++i) {
            ...
            uint256 supplyShares = CANAL.supplyShares(id, address(this));
            (uint256 totalSupplyAssets, uint256 totalSupplyShares,,) = CANAL.expectedMarketBalances(_marketParams(id));
    ...
    /// @inheritdoc ERC4626
    /// @dev Used in mint or deposit to deposit the underlying asset to Canal markets.
    ...
    /// @inheritdoc ERC4626
    /// @dev Used in redeem or withdraw to withdraw the underlying asset from Canal markets.
    ...
    function _marketParams(Id id) internal view returns (MarketParams memory) {
        return CANAL.idToMarketParams(id);
    }

    /// @dev Accrues interest on Canal Blue and returns the vault's assets & corresponding shares supplied on the
    ...
    {
        CANAL.accrueInterest(marketParams);

        market = CANAL.market(id);
        shares = CANAL.supplyShares(id, address(this));
    ...
    function _setCap(MarketParams memory marketParams, Id id, uint184 supplyCap) internal {
        ...
        if (supplyCap > 0) {
            if (!marketConfig.enabled) {
                ...
                _updateLastTotalAssets(lastTotalAssets + CANAL.expectedSupplyAssets(marketParams, address(this)));
    ...
    /// @dev Supplies `assets` to Canal.
    function _supplyCanal(uint256 assets) internal {
        for (uint256 i; i < supplyQueue.length; ++i) {
            Id id = supplyQueue[i];
            ...
            if (toSupply > 0) {
                ...
                try CANAL.supply(marketParams, toSupply, 0, address(this), hex"") {
                    assets -= toSupply;
                } catch {}
            }
    ...
    /// @dev Withdraws `assets` from Canal.
    function _withdrawCanal(uint256 assets) internal {
        for (uint256 i; i < withdrawQueue.length; ++i) {
            Id id = withdrawQueue[i];
            MarketParams memory marketParams = _marketParams(id);
            ...
            if (toWithdraw > 0) {
                // Using try/catch to skip markets that revert.
                try CANAL.withdraw(marketParams, toWithdraw, 0, address(this), address(this)) {
                    assets -= toWithdraw;
                } catch {}
            }
    ...
    function _simulateWithdrawMorpho(uint256 assets) internal view returns (uint256) {
        for (uint256 i; i < withdrawQueue.length; ++i) {
            ...
            (uint256 totalSupplyAssets, uint256 totalSupplyShares, uint256 totalBorrowAssets,) =
                CANAL.expectedMarketBalances(marketParams);

            // The vault withdrawing from Canal cannot fail because:
            // 1. oracle.price() is never called (the vault doesn't borrow)
            // 2. the amount is capped to the liquidity available on Canal
    ...
    function _withdrawable(
        ...
        // Inside a flashloan callback, liquidity on Canal Blue may be limited to the singleton's balance.
        uint256 availableLiquidity = UtilsLib.min(
            totalSupplyAssets - totalBorrowAssets, ERC20(marketParams.loanToken).balanceOf(address(CANAL))
        );
